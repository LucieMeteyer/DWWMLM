                    ...
                    if ($result = mysqli_query($link, $sql)) {
                        if (mysqli_num_rows($result) > 0) {
                            ...
                            while ($row = mysqli_fetch_array($result)) {
                                ...
                                echo '<td>' . $row["role"] . '</td>';
                                echo '<td>';
                                echo '<a href="read.php?id=' . $row
                                ["id"] .'"class="mr-3" title="vue" 
                                data toggle="tooltip"><span class="fa 
                                fa-eye"></span></a>';
                                echo '<a href="update.php?id=' . $row
                                ["id"] .'"class="mr-3" title="mise a
                                 jour" data toggle="tooltip"><span class="fa fa-pencil"></span></a>';
                                echo '<a href="delete.php?id=' . $row
                                ["id"] .'"class="mr-3" title="delete" 
                                data toggle="tooltip"><span class="fa 
                                fa-trash"></span></a>';
                                echo '</td>';
                                echo '</tr>';
